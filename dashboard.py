...
from dashboard_func import load_ticks_db, load_diagnostics, plot_price, plot_ticks_per_second, plot_websocket_lag
...
with tab2:
    ...
    if not websocket_avg_df.empty:
        avg_websocket_lag = df['websocket_lag'].mean()
    ...
    processing_df = load_diagnostics("processing_diagnostics", limit=100, order_col="processed_timestamp")

    st.write("How long does our pipeline take to process data before it reaches the database?")
    if not processing_df.empty:
        avg_processing_lag = df['processing_lag'].mean()
        st.metric(label="Average Processing Lag (in Seconds):", value=round(avg_processing_lag, 2))
        st.plotly_chart(plot_websocket_lag(websocket_avg_df, "Average Processing Lag (in Seconds)"), use_container_width=True)
